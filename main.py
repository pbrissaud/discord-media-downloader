...
import requests, os, sys, docker, getopt
...
def main(argv):
    ...
    try:
        opts, _ = getopt.getopt(
            argv, "hi:t:o:v",
            ["help", "input=", "token=", "output=", "verbose"])
    ...
    with open(inputFilePath) as inputFile:
        for line_number, line in enumerate(inputFile, 1):
            try:
                ...
                print("ERROR --- Incorrect entry at line " + str(line_number) +
                      " of input file : No ':' found")
                sys.exit(5)
            value = value.rstrip()
            if value == '':
                print("ERROR --- Incorrect entry at line " + str(line_number) +
                      " of input file : Empty channel ID")
                sys.exit(5)
    ...
    if outputDirectory == '':
        ...
        print(
            "INFO  --- Output directory not specified, create 'export' folder in ",
            pwd) if _verbose == True else None
    else:
        if not os.path.exists(outputDirectory):
            print(
                "ERROR --- The output directory specified in parameter doesn't exist"
            )
            sys.exit(6)
        if not os.path.isdir(outputDirectory):
            print(
                "ERROR ---  The output directory specified in parameter isn't a directory"
            )
            sys.exit(7)
    ...
    if token == '':
        print(
            "INFO  --- The Discord token wasn't specified in parameter, searching in env DISCORD_TOKEN"
        ) if _verbose == True else None
        token: str = os.getenv('DISCORD_TOKEN')
        if token == None:
            print(
                "ERROR --- The Discord token wasn't specified either in parameter or in environnement variable"
            )
            usage()
            sys.exit(8)
...
def run(discordToken, channelsDict, outputDirectory):
    ...
    for channelName, channelID in channelsDict.items():
        print("INFO  --- Exporting " + channelName + " channel (id=" +
              channelID + ")")
        ...
        client.containers.run("tyrrrz/discordchatexporter:stable",
        ...
        if len(files) == 0:
            print(
                "ERROR --- The container didn't export the channel, meaning that your token or the channel id is invalid"
            )
        ...
        print("INFO  --- Found " + str(len(attachementDivs)) +
              " medias in channel " + channelName)
        i = 0
            ...
            if not os.path.exists(path):
                ...
                with open(path, "wb") as f:
                    print("INFO  --- Downloading ",
                          attachement) if _verbose == True else None
...
def usage():
    ...
    print(
        "-o, --output\tNO\t\tPath to directory where attachements will be downloaded ; if not specified, the script will create one in pwd"
    )
    print(
        "-t, --token\tNO\t\tDiscord user token ; if not specified, the script will search in DISCORD_TOKEN env\n\t\t\t\tOne of two must be defined (the script always take the parameter first)\n\t\t\t\tTo know how to get a user token, visit https://github.com/pbrissaud/discord-attachements-exporter/wiki/Usage-guide#grab-your-discord-token"
    )
